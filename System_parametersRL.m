...
p(10) = 1/5; % mosquito biting rate. It seems reasonable to assume a female mosquito will take a blood meal every 5 days.
             % Cite Ruybal, Geographic variation in the response of Culex pipiens life history traits to temperature
...
p(26) = 0; % WNV death host group 1 - DONE
...
% (1-max_ef)(p(7)+p(8)+p(38)) = (p(7)+p(8))
% (1-max_ef) p(38) = max_ef(p(7)+p(8))
% p(38) = max_ef*(p(7)+p(8))/(1-max_ef)

% After half_ef_day, the larvicide induced death rate ul*p(38) should satisfy ul(half_ef_day)*p(38)=p(8)+p(7).
% (That is, the number of adults emerging is reduced by 50%.) 
% So ul(half_ef_day)*max_ef/(1-max_ef) = 1, or ul(half_ef_day) = (1-max_ef)/(max_ef) = exp(-half_ef_day*p(39))
% p(39)=-log[(1-max_ef)/(max_ef)]/half_ef_day
...
% p(38) satisfies p(7)/(p(7)+p(8)+p(38))= (1-max_ef) p(7)/(p(7)+p(8)) 
% (1-max_ef)(p(7)+p(8)+p(38)) = (p(7)+p(8))
% (1-max_ef) p(38) = max_ef(p(7)+p(8))
% p(38) = max_ef*(p(7)+p(8))/(1-max_ef)

% After mid_ef_day, the larvicide induced death rate ul*p(38) should satisfy (1-mid_eff)*[ul(mid_ef_day)*p(38)+p(8)+p(7)]=(p(8)+p(7)).
% (That is, the number of adults emerging is reduced by mid_eff*100%.) 
% So ul(mid_ef_day)*p(38)=[(mid_eff)/(1-mid_eff)](p(8)+p(7))
% ul(mid_ef_day)*max_ef*(p(7)+p(8))/(1-max_ef)=[(mid_eff)/(1-mid_eff)](p(8)+p(7))
% ul(mid_ef_day)=[(mid_eff)(1-max_ef)]/[(1-mid_eff)max_ef], 
% [(mid_eff)(1-max_ef)]/[(1-mid_eff)max_ef] = exp(-mid_ef_day*p(39))
% p(39)=-log([(mid_eff)(1-max_ef)]/[(1-mid_eff)max_ef])/mid_ef_day

% If we require the control be only min_ef=64% effective after min_ef_day days 
% p(7)/[ul(min_ef_day)*p(19)+p(7)+p(8)] = (1-min_ef)* p(7)/[p(7)+p(8)]
% [ul(min_ef_day)*p(38)]=min_ef*(p(7)+p(8))/(1-min_ef)
% exp(-min_ef_day*p(17))*max_ef*(p(8)+p(7))/(1-max_ef) = min_ef*(p(7)+p(8))/(1-min_ef)
...
% dV/dt = -p(40)*A_0*exp(-p(41)*t)*V
% log(V(t)/V_0)=p(40)*A_0*exp(-p(41)*t)/p(41)-p(40)*A_0/p(41)
% taking limit as t goes to inifinity
% log(0.1)=-p(40)*A_0/p(41)
% p(40)=-log(0.1)*p(41)/A_0
% A_0=1/2.
